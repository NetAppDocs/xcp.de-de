...
NOTE: Mit der Option -stats wird ein vom Benutzer lesbarer Bericht an die Konsole gedruckt. Andere Berichtformate sind -HTML oder -csv. Das CSV-Format (Comma-Separated Values) weist genaue Werte auf. CSV- und HTML-Berichte werden automatisch im Katalog gespeichert, wenn ein Katalog vorhanden ist.
