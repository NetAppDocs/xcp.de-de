...
summary: XCP SMB „help“-Befehlsreferenz 
---
= Hilfe
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Das SMB `help` Befehl zeigt eine Liste von Befehlen, Befehlsparametern und eine kurze Beschreibung der einzelnen Befehle an. Dieser Befehl ist sehr nützlich für Anfänger, die neu in XCP sind.
...
Verwenden Sie <command> mit `help` So zeigen Sie Beispiele und Optionsdetails für die angegebene <command> an.
...
xcp help <command>
----
In der folgenden Beispielausgabe werden Details, Verwendung, Argumente und optionale Argumente für den angezeigt `sync` Befehl.
