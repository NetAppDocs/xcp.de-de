...
[role="lead"]
Das NFS `help` Befehl zeigt eine Liste von Befehlen, Befehlsparametern und eine kurze Beschreibung der einzelnen Befehle an. Der `help` Command ist nützlich für Anfänger, die neu im XCP-Tool sind.
...
Verwenden Sie die `info` Parameter mit `help` Befehl zum Anzeigen der Dokumentation, Beispiele und Optimierungsempfehlungen.
