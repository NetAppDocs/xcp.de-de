...
[NOTE]
====
* Die `-stats` Option druckt einen für den Benutzer lesbaren Bericht an die Konsole. Weitere Optionen für das Berichtsformat sind `-html` oder `-csv`. Das CSV-Format (Comma-Separated Values) weist genaue Werte auf. CSV- und HTML-Berichte werden automatisch im Katalog gespeichert, wenn ein Katalog vorhanden ist.
* XCP-Berichte (.csv, .HTML) werden an dem in der Datei angegebenen Katalogspeicherort gespeichert `xcp.ini`. Die Dateien werden im Ordner gespeichert `<catalog path>/catalog/indexes/1/reports`. Sie können Beispielberichte im anzeigenlink:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886872["NetApp XCP 1.9.3 Referenz"^].
